...
namespace os
{
	...
	ITimer::RealTimeDate Timer::getRealTimeAndDate()
	{
		...
		date.Hour = static_cast<u32>(timeinfo.tm_hour);
		date.Minute = static_cast<u32>(timeinfo.tm_min);
		date.Second = static_cast<u32>(timeinfo.tm_sec);
		date.Day = static_cast<u32>(timeinfo.tm_mday);
		date.Month = static_cast<u32>(timeinfo.tm_mon + 1);
		date.Year = static_cast<u32>(timeinfo.tm_year + 1900);
		date.Weekday = static_cast<ITimer::EWeekday>(timeinfo.tm_wday);
		date.Yearday = static_cast<u32>(timeinfo.tm_yday + 1);
